...
angular.module('datasite')
  ...
  .service('Spec', function(ANY, _, vg, vl, cql, util, ZSchema, consts,
      Alerts, Chart, Config, Dataset, Feed, Logger, Pills, Visquery, Schema, FilterManager) {
    ...
    Spec.update = function(spec) {
      ...
      if (!('config' in spec)) {
        ...
        Spec.isEmptyPlot = false;
        Spec.isSpecific = true;
        Spec.feeds = [];
        // Spec.alternatives = [];
        ...
        // var currentChart = {
        //   enumSpecIndex: 0,
        //   fieldSet: query.spec.encodings,
        //   vlSpec: query.spec,
        //   shorthand: null
        // };
        // if (Spec.isSpecific || Spec.isEmptyPlot) {
        //   // Spec.chart = Chart.getChart(topItem);
        //   Spec.chart = currentChart;
        //   Spec.charts = null;

        //   if (Dataset.schema) {
        //     // modified by Zhe
        //     // Make Feed independent of the user interactions
        //     Spec.feeds = Feed.getVisualization(query, Spec.chart);
        //     // Spec.feeds = Feed.getVisualization(query, Spec.chart, topItem);
